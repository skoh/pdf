...
public class PathConstructionAndPainting {
	...
	/** The resulting PDF. */
	public static final String RESULT = "results/part4/paths.pdf";
	...
	public void createPdf(String filename) throws IOException, DocumentException {
		...
		ColumnText.showTextAligned(canvas, Element.ALIGN_LEFT, new Phrase(
				"Methods moveTo(), lineTo(), stroke(), closePathStroke(), fill(), and closePathFill()"), 50, 700, 0);
		// draw Bezier curves
		createBezierCurves(canvas, 70, 600, 80, 670, 140, 690, 160, 630, 160);
		ColumnText.showTextAligned(canvas, Element.ALIGN_LEFT, new Phrase(
				"Different curveTo() methods, followed by stroke()"), 50, 580, 0);
		// draw stars and circles
		createStarsAndCircles(canvas, 50, 470, 40, 20);
		ColumnText.showTextAligned(canvas, Element.ALIGN_LEFT, new Phrase(
				"Methods fill(), eoFill(), newPath(), fillStroke(), and eoFillStroke()"), 50, 450, 0);
		...
		ColumnText.showTextAligned(canvas, Element.ALIGN_LEFT, new Phrase("Convenience methods"), 50, 250, 0);
		// step 5
		document.close();
	}
	 ...
	 */
	public void createSquares(PdfContentByte canvas, float x, float y, float side, float gutter) {
		canvas.saveState();
		canvas.setColorStroke(new GrayColor(0.2f));
		canvas.setColorFill(new GrayColor(0.9f));
		canvas.moveTo(x, y);
	 ...
	 */
	public void createBezierCurves(PdfContentByte cb, float x0, float y0, float x1, float y1, float x2, float y2,
	 ...
	 */
	public static void createStarsAndCircles(PdfContentByte canvas, float x, float y, float radius, float gutter) {
		canvas.saveState();
		canvas.setColorStroke(new GrayColor(0.2f));
		canvas.setColorFill(new GrayColor(0.9f));
	 ...
	 */
	public static void createCircle(PdfContentByte canvas, float x, float y, float r, boolean clockwise) {
		float b = 0.5523f;
		if (clockwise) {
			canvas.moveTo(x + r, y);
